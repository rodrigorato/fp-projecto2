...
def jogada_faz_algo(tab, jogada):
    """Teste: recebe um tabuleiro e uma jogada - devolve True se o tabuleiro for alterado pela jogada ou False em caso contrario (nao adiciona outra peca de forma aleatoria)"""
    tab_depois_da_jogada = copia_tabuleiro(tab)
    tab_depois_da_jogada = tabuleiro_reduz(tab_depois_da_jogada, jogada)
    return not tabuleiros_iguais(tab, tab_depois_da_jogada)
...
def jogo_2048():
    ...
    jogada = 'N'                            #jogada valida de valor arbitrario
    tab = preenche_posicao_aleatoria(tab)             
    while not tabuleiro_terminado(tab):
        if jogada_faz_algo(tabuleiro_anterior, jogada) or \
           tabuleiros_iguais(tabuleiro_anterior, cria_tabuleiro()):    
            #verifica se a jogada anterior alterou o tabuleiro - se nao alterou nao adicionamos outra peca
